        ...
        if (xQueueReceive(output->m_i2sQueue, &evt, portMAX_DELAY) == pdPASS)
        {
            if (evt.type == I2S_EVENT_TX_DONE)
            {
                ...
                do
                {
                    if (availableBytes == 0)
                    {
                        // //Print how many bytes has been completed
                    ...
                    // if(buffer_position >= 2048) {
                    //     //kill the task if we have written 2k bytes
...
void I2SOutputWAV::start(i2s_port_t i2sPort, i2s_pin_config_t &i2sPins, SampleSource *sample_generator)
{
